<template>
    <template v-if="tracker !== null">
        <Dialog title="Edit Data Entry" :open="data !== null" @close="onClose" @confirm="handleSubmit">
...
const value = ref<number>(props.data?.value ?? 0)
...
const handleSubmit = async () => {
    ...
    try {
        const response = await $api<ApiResponse>(`/trackers/entry/${props.data!.id}`, {
            method: "PATCH",
            body: JSON.stringify({
                value: value.value = parseFloat(String(value.value))
            })
        ...
        throw new Error("Failed to edit tracker data")
    } catch (e: any) {
