    ...
    <Dialog @close="onClose" @confirm="handleSubmit" :open="props.open" title="Add New To-Do">
        <div class="flex flex-col gap-y-4">
            ...
            <div>
                ...
                <Input type="date" :min="dateMinimum" :max="dateMaximum" v-model="date" />
            </div>
            <div>
                <label for="hour">Time</label>
                <div class="flex gap-x-4 items-center">
                    <select :value="hourDefaultValue.toString()" id="hour" name="hour"
                      class="px-2 cursor-pointer border border-gray-800 rounded" v-model="hour">
                    ...
                    <select value="0" id="minute" name="minute"
                      class="px-2 cursor-pointer border border-gray-800 rounded" v-model="minute">
                        <option v-for="minute in minutes" :key="minute" :value="minute.toString()">{{
                            padZeroLeft(minute, 2) }}
                        </option>
                    </select>

                    <select :value="ampmDefault" id="ampm" name="ampm"
                      class="px-2 cursor-pointer border border-gray-800 rounded" v-model="ampm">
...
const now = new Date()
const dateMinimum = now.toISOString().split('T')[0]
const year = now.getFullYear()
const month = now.getMonth() + 1
const day = now.getDate()

const dateMinimumString = `${year}-${padZeroLeft(month, 2)}-${padZeroLeft(day, 2)}`;
const dateMaximum = new Date(now.setFullYear(now.getFullYear() + 1)).toISOString().split('T')[0]

const props = defineProps<Props>()
const hourDefaultValue = now.getHours() % 12 == 0 ? 12 : new Date().getHours() % 12

const minutes = [0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55]

const ampmDefault = now.getHours() < 12 ? "am" : "pm"

const minuteDefault = now.getMinutes() - (now.getMinutes() % 5)
